...
def transrot(batches):
    ...
    with aff.pool(processes=3, maxtasksperchild=12, seed=1) as pool:
...
def gaussian(batches):
    with iaa.GaussianBlur(sigma=(0.0, 3.0)).pool(processes=3, maxtasksperchild=12, seed=1) as pool:
        batches_aug = pool.map_batches(batches)
    for i in range(len(filenames_result)):
...
def brightcont(batches):
    ...
    with augm.pool(processes=3, maxtasksperchild=12, seed=1) as pool:
        batches_aug = pool.map_batches(batches)
    for i in range(len(filenames_result)):
...
def saltpepper(batches):
    with iaa.SaltAndPepper(0.1).pool(processes=3, maxtasksperchild=12, seed=1) as pool:
        batches_aug = pool.map_batches(batches)
    for i in range(len(filenames_result)):
